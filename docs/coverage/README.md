...
| `v6/authored/Voting.status_updates.json` | authored | 9 | The statusUpdates section (post-RuleStatusSerializer-port): canonical activation round-trips (single + the real mainnet 3×ReplacedRule shape) · unknown-statusCode forward-compat skip (plain boundary; activation unrepresentable — the ReplacedRule(0) re-serialization landmine, reported) · trailing-bytes accept with canonical drop · count-wrap 0xFFFFFFFF ⇒ empty on both counts · truncated-dataBytes / dataSize-over-claim / bare-00 rejects |
