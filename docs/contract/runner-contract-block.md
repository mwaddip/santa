...
| **donner** | full — the tier's real conformer (`cost: true`) | ergo-node-rust's `validation` crate over its digest-state seam (`DigestValidator::from_state` + `BlockValidator::apply_state`), mounted as the `runners/donner` submodule (santa-donner repo; `impl` clones ergo-node-rust). enr surfaces block cost and enforces the maxBlockCost sum. Build-identity (§3 of the core contract) has one donner-specific amendment (2026-08-04): its `[patch.crates-io]` table is **derived by `santa-run` from the staged enr's own manifest**, copied verbatim, rather than hand-restated. Cargo honours `[patch]` only from the consuming workspace root, so donner's table governs enr's own crates and every enr entry must be restated — hand-restating failed twice (`ergo_avltree_rust` drifted 16 commits by rev; `redb` was never restated at all and resolved to crates.io for three weeks, leaving donner *ungraded* rather than red). Derivation makes the staged enr the single source of truth, so a green donner cell means "enr commit X built against exactly the dependency closure X declares", reproducible by checking out X. It **fails the run** on any entry it cannot resolve rather than omitting it — the redb failure was quiet enough to look like success, and a silent skip would reproduce that. The committed table between the `DERIVED-PATCH-TABLE` markers is a snapshot for direct `cargo build` only. |
