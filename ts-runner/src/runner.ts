 ...
 *  Every non-null tag has value:null, cost:null. Scope (which entries a conformer is
 *  expected to satisfy) is selected on the INPUT side (which vector dirs are run) and
 *  judged downstream — never here. */
type Result = {
  value: Json
  cost: number | null
...
/** Run one entry → exactly one Result. */
export function runEntry(schema: string, e: Entry): Result {
  ...
  //     not-implemented; ANY other parse failure is a loud bug (re-throw), NOT errored.
  let tree
  try {
  ...
  //     (Header ts overflow ⇒ ReaderError 'vlq-overflow') ⇒ unrepresentable; else ⇒ re-throw.
  let ctx
  try {
