...
| **4 — block tier** | digest-state reframe: `santa-block/v1` (parent digest + headers + params + block-with-ADProofs) · 3 captured seeds ADProofs-verified + 6 authored mutation classes · gated `BlockEngine` + rudolph control row · donner (ergo-node-rust) routed | 🟢 live (donner pending) |
