...
| **Chain** | header-chain-decidable consensus functions — difficulty retargeting + parameter-voting math + fork-vote gate; value-only; versions v5/v6/any | the chain (captured) · the JVM reference (authored edge cases) | any conformer that computes retargeting or param-voting (rudolph control; donner the natural first independent; blitzen nipopow-at-most deferred); contract: [`docs/contract/runner-contract-chain.md`](docs/contract/runner-contract-chain.md) |
| **AuthDS** | authenticated-data-structure functions directly, below ErgoScript — AVL+ batch **prove** (`avl_prove`: canonical proof bytes + digests) and **verify** (`avl_verify`: proof-accept + per-op results + digest); value-only; version `any` | scrypto 3.0.0's `BatchAVLProver`/`BatchAVLVerifier` (via `jvm-blesser`) | rudolph control + dasher (ergots) live; eni/donner verify-only when mounted (sigma-rust has no library prover; donner's is a SANTA-side dump tool); vixen pending; contract: [`docs/contract/runner-contract-authds.md`](docs/contract/runner-contract-authds.md) |
| **NiPoPoW** | synthetic chain → interlinks + proof bytes (non-continuous; header-chain-decidable) | JVM `NipopowAlgos` (ergo-core) | any conformer with `updateInterlinks` + `prove` (rudolph control; dasher, blitzen-eni, donner, vixen pending); contract: [`docs/contract/runner-contract-nipopow.md`](docs/contract/runner-contract-nipopow.md) |
...
corpus at scale**, since grown by authored gap-fillers: **2,346 entries across 211 files**
...
The **chain tier is live** — `santa-chain/v1`; **14 files / 61 entries** spanning three kinds (retargeting + parameter-voting + fork-vote-gate) across four slices (`any/captured` 2 · `any/authored` 3 · `v6/captured` 1 · `v6/authored` 55), value-only (no cost dimension at this tier); the voting and fork-vote-gate `v6/authored` slices also carry a **reject arm** (`expected.error: "errored"`, contract §2) covering hostile input classes. Corpus: `Retargeting.testnet_points` (2 captured recalculation points, targets 393601/393473 — engine FAIL-LOUD-equal to the on-chain headers' nBits) · `Retargeting.damping_clamps` (flat-control + both damping clamps; the 0.5×/1.5× clamps live only in `eip37Calculate`, so these carry the eip37 settings pair — the settings-driven EIP-37 dispatch is exercised; a mainnet EIP-37-era captured window stays a gap) · `Voting.testnet_epoch_2560` (the real boundary, identity epoch — table equality is the pin, engine FAIL-LOUD-equal to `parseExtension`) · `Voting.threshold_edges` (4 entries: half = no step under strict `>` · half-plus-one steps exactly id 1 · fork votes without an in-progress round count for nothing · id-9 steppability) · `Voting.window_clamp` (chain-start clamp ⇒ empty seed ⇒ unseeded votes drop) · `Voting.softfork_round` (7 entries: round lifecycle — start/accumulate/last-accumulate/wait-identity/failed-cleanup/failed-restart/midround-forkvote-noop) · `Voting.softfork_activation` (8 entries: activation basis yes/no + id-9 insertion/409-disable/testnet-proposal/sigma-rule pass-through/cleanup/cleanup-restart) · `Voting.softfork_zombie` (4 entries: zombie checkpoint-flips — survive/no-activation/late-cleanup/stuck-forever; see [`docs/findings/chain-softfork-zombie-liveness.md`](docs/findings/chain-softfork-zombie-liveness.md)) · `Voting.hostile_tables` (3 entries: reject classes — 122-without-121/unknown-id-approved/mandatory-rule-update) · `Voting.lifecycle_leniency` (4 entries, enr's cross-read asks: the LAZY-votes pass-through inverse of the 122-without-121 hostile · orphan-121 inertness + its fork-vote overwrite · the Int-wrap of `votes`) · `Voting.status_updates` (9 entries, unlocked by enr's RuleStatusSerializer port: the mainnet h=1,628,160 3×ReplacedRule shape + a single ReplacedRule round-trip CANONICALLY at activation · the unknown-statusCode forward-compat skip (plain boundary — the activation arm is UNREPRESENTABLE: re-serializing ReplacedRule(0) computes offset −1000 and putUShort require-fails, the JVM cannot re-serialize what it accepted; reported) · trailing-bytes accept + canonical drop · getUInt().toInt count-wrap on BOTH counts ⇒ empty update · 3 underflow rejects) · `Voting.tally_order` (3 entries: the ordered-Array tally — contradictory ±1 seed-slot pairs pin last-write-wins from the post-fork SNAPSHOT (on-chain-unreachable, legality-upstream) · duplicated-120 seed pins find-first vs summing at a checkpoint) · `ForkVoteGate.window_edges` (8 entries: window-edge cases incl. the 3686→3687 height-flip + during-voting leniency — gate open during the vote window, closed outside) · `ForkVoteGate.preconditions` (4 entries: 120-gating contract — non-120 id passes, 122-without-121 blocked before the table; read-order pair — preconditions check before window; eager-`.get` reject). Oracle: gated `ChainEngine` (ergo-core `DifficultyAdjustment` / `Parameters.update`), every captured expected cross-checked against chain history at bless time. Contract: [`docs/contract/runner-contract-chain.md`](docs/contract/runner-contract-chain.md). Conformers: **rudolph control** 52/52 (all 4 chain slices green, incl. fork_vote_gate 12/12) · **donner LIVE** 52/52 — converged green on the full 3-kind corpus day one; the routing round that preceded this batch converted the heads-up into a fix-before-first-grade: enr's plain-counter epoch tally was a REAL consensus bug on its live boundary path (`count_votes_in_epoch` fed `compute_expected_parameters`), replaced by seeded semantics, plus three further JVM-exactness finds their cross-read surfaced (approval counts = closing-epoch-120 PLUS collected · lifecycle reads an original-table snapshot · approved-vote-for-unknown-id errors like the JVM · id 9 steppable) · **vixen LIVE** — 27/34 chain/v6/authored (zombie 4/4 + tally-order + Int-wrap + lazy-leniency natively exact; 7 reds = restart snapshot/reset leaks + 3 hostile-reject gaps, routed); fork_vote_gate 12 entries graded **not-implemented** — their ledger until they mount the gate arm (santa-vixen `80d4473` / arkadianet `fa97cfc`, same-day mount; natively-seeded tally walker, JVM-pair-returning `compute_next_params`; one recorded caveat — `use_last_epochs` hardcoded 8, not entry-threaded; their NiPoPoW is an OWN implementation ⇒ vixen-mountable when that kind ships) · blitzen nipopow-at-most deferred · dasher ledger.
The **authds tier is live** — `santa-authds/v1`, grading the AVL+ batch prover and verifier
directly, below the ErgoScript layer the eval tier reaches them through, against scrypto
3.0.0 via `jvm-blesser`. **60 vendored entries** (`vectors/authds/any/vendored/` — 10
`avl_prove` + 50 `avl_verify`, ergots' own AVL-prover fixture corpus, every expectation
re-derived through the JVM rather than trusted from its Rust-blessed origin). Conformers:
**rudolph control** red 0 (`proof 10/10 · digest 56/56 · accepted 50/50 · results 46/46`) ·
**dasher (ergots)** red 24 (`proof 0/10 · digest 42/52 · accepted 50/50 · results 42/46`) —
two confirmed findings (pushed ergots `master` seeds the empty AVL tree with an extra
sentinel leaf, fixed locally but unpushed, all 10 `avl_prove` entries; scrypto's
`UnknownModification` is a fixed-zero-length-key singleton where ergots/`ergo_avltree_rust`
treat it as keyed, 4 `avl_verify` results —
[finding](docs/findings/authds-unknownmodification-jvm-vs-rust.md)) plus one predicted
`adverse-*` reject-detection miss that came back green instead. blitzen-eni / donner /
vixen `avl_verify` arms routed, not yet mounted. Contract:
