...
| **4 — block tier** | digest-state reframe: `santa-block/v1` (parent digest + headers + params + block-with-ADProofs) · 3 captured seeds ADProofs-verified + 5 authored mutation classes (version-gate pending an epoch-boundary re-donor) · gated `BlockEngine` + rudolph control row · **donner (ergo-node-rust) + vixen (arkadianet/ergo) both grading** | ✅ live (7-way) |
