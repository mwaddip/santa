...
The **chain tier is live** — `santa-chain/v1`; **9 files / 33 entries** spanning two kinds (retargeting + parameter-voting) across four slices (`any/captured` 2 · `any/authored` 3 · `v6/captured` 1 · `v6/authored` 27), value-only (no cost dimension at this tier); the voting `v6/authored` slice also carries a **reject arm** (`expected.error: "errored"`, contract §2) covering hostile input classes. Corpus: `Retargeting.testnet_points` (2 captured recalculation points, targets 393601/393473 — engine FAIL-LOUD-equal to the on-chain headers' nBits) · `Retargeting.damping_clamps` (flat-control + both damping clamps; the 0.5×/1.5× clamps live only in `eip37Calculate`, so these carry the eip37 settings pair — the settings-driven EIP-37 dispatch is exercised; a mainnet EIP-37-era captured window stays a gap) · `Voting.testnet_epoch_2560` (the real boundary, identity epoch — table equality is the pin, engine FAIL-LOUD-equal to `parseExtension`) · `Voting.threshold_edges` (4 entries: half = no step under strict `>` · half-plus-one steps exactly id 1 · fork votes without an in-progress round count for nothing · id-9 steppability) · `Voting.window_clamp` (chain-start clamp ⇒ empty seed ⇒ unseeded votes drop) · `Voting.softfork_round` (7 entries: round lifecycle — start/accumulate/last-accumulate/wait-identity/failed-cleanup/failed-restart/midround-forkvote-noop) · `Voting.softfork_activation` (8 entries: activation basis yes/no + id-9 insertion/409-disable/testnet-proposal/sigma-rule pass-through/cleanup/cleanup-restart) · `Voting.softfork_zombie` (4 entries: zombie checkpoint-flips — survive/no-activation/late-cleanup/stuck-forever; see [`docs/findings/chain-softfork-zombie-liveness.md`](docs/findings/chain-softfork-zombie-liveness.md)) · `Voting.hostile_tables` (3 entries: reject classes — 122-without-121/unknown-id-approved/mandatory-rule-update). Oracle: gated `ChainEngine` (ergo-core `DifficultyAdjustment` / `Parameters.update`), every captured expected cross-checked against chain history at bless time. Contract: [`docs/contract/runner-contract-chain.md`](docs/contract/runner-contract-chain.md). Conformers: **rudolph control** 33/33 (all 4 chain slices green) · **donner LIVE** — first batch 10/10 (santa-donner `aaca7cf` / enr `9ccc6e7`); the soft-fork batch grades 8/27 (19 reds = the handed-121/122 lifecycle gap + the mandatory-rule parse, routed 2026-06-11) — the routing round converted the heads-up into a fix-before-first-grade: enr's plain-counter epoch tally was a REAL consensus bug on its live boundary path (`count_votes_in_epoch` fed `compute_expected_parameters`), replaced by seeded semantics, plus three further JVM-exactness finds their cross-read surfaced (approval counts = closing-epoch-120 PLUS collected · lifecycle reads an original-table snapshot · approved-vote-for-unknown-id errors like the JVM · id 9 steppable) · **vixen LIVE both kinds** — first batch 10/10; the soft-fork batch grades 21/27 (zombie 4/4 natively exact; 6 reds = 3 restart snapshot leaks + 3 hostile-reject gaps, routed 2026-06-11) (santa-vixen `80d4473` / arkadianet `fa97cfc`, same-day mount; natively-seeded tally walker, JVM-pair-returning `compute_next_params`; one recorded caveat — `use_last_epochs` hardcoded 8, not entry-threaded; their NiPoPoW is an OWN implementation ⇒ vixen-mountable when that kind ships) · blitzen nipopow-at-most deferred · dasher ledger.
