...
- ✅ **A blessed eval corpus — 2,282 entries across 187 vector files**: 2,026 produced by
  the JVM reference interpreter (`sigma-state`) from its own language specification, plus
  256 authored gap-fillers (oracle-blessed, never spec-copied); version-split into **v5**
  (1,880 entries — the cumulative v5/mainnet method surface)
  and **v6** (402 — the v6 new-feature surface). Each entry is `ErgoTree bytes (+ input)
